 ...
 * 
 * upgrade auth: monlam
 * augrade version: 2023-05-27
 * 增加动态加载options数据参数 url, 及对应数据处理方法 urlLoad
 */
layui.define(['jquery', 'http'], function (exports) {
    ...
    // 模块名
    const MODULE_NAME = 'cascader'
    // 加载模块资源
    const modFile = layui.cache.modules[MODULE_NAME];
    const modPath = modFile.substr(0, modFile.lastIndexOf('/'))
    layui.link(modPath + '/cascader.css?v=' + (typeof window.version == 'string' ? window.version : Math.random()));
    ...
    Node.prototype = {
        ...
        set loading(loading) {
            let $li = this.$li;
            if ($li) {
                let rightIcon = this.icons.right;
                let loadingIcon = this.icons.loading;
                let $i = $li.find('i');
                if (loading) {
        ...
        get disabled() {
            ...
            let maxSize = this.config.maxSize;
            let checkedNodeIds = this.cascader.data.checkedNodeIds;
            let disabledName = this.props.disabled;
            let checkStrictly = this.props.checkStrictly;
            // 检查是否超过最大值限制
            if (multiple && maxSize !== 0) {
                if (checkedNodeIds.length >= maxSize && checkedNodeIds.indexOf(this.nodeId) === -1) {
                    // 如果是关联的多选，需要查询叶子节点是否有被选中的项
                    if (!checkStrictly) {
                        let leafChildren = this.getAllLeafChildren();
                        let nodeIds = leafChildren.map(function (value) {
                            return value.nodeId
                        });
                        // 如果叶子节点不包含，则直接返回true
        ...
        /** 叶子节点 */
        get leaf() {
            let leaf = this.data[this.props.leaf];
        ...
        get $tag() {
            ...
            let showAllLevels = this.config.showAllLevels;
            let disabled = this.config.disabled;
            let nodeDisabled = this.disabled;
            let disabledFixed = this.config.disabledFixed;

            let label = this.getPathLabel(showAllLevels);
            let $tag = cascader.get$tag(label, !disabled && (!nodeDisabled || !disabledFixed));
            let self = this;
        ...
        initSuggestionLi: function () {
            let label = this.getPathLabel(true);
        ...
        _renderFiltering: function () {
            let $li = this.$suggestionLi;
            let nodeId = this.nodeId;
            let fromIcon = this.icons.from;
            let okIcon = this.icons.ok;
            let self = this;
            let cascader = this.cascader;
            let multiple = this.props.multiple;

            let icon = '<i class="' + fromIcon + ' ' + okIcon + ' el-icon-check"></i>';
        ...
        _renderRadio: function () {
            ...
            let self = this;
            let cascader = this.cascader;
            let activeNode = this.cascader.data.activeNode;
            let parentNode = this.parentNode;
            ...
            this._liClick(function (event) {
                stopPropagation(event);
                let childrenNode = self.childrenNode;
        ...
        _renderRadioCheckStrictly: function () {
            ...
            let self = this;
            let cascader = this.cascader;
            let activeNode = cascader.data.activeNode;
            let parentNode = this.parentNode;

            $li.addClass('is-selectable');
            // 任意一级单选
            let $radio = $('<label role="radio" tabindex="0" class="el-radio"><span class="el-radio__input"><span class="el-radio__inner"></span><input type="radio" aria-hidden="true" tabindex="-1" class="el-radio__original" value="' + nodeId + '"></span><span class="el-radio__label"><span></span></span></label>');
            this.$radio = $radio;
            $li.prepend($radio);
            if (parentNode) {
                parentNode.$li.siblings().removeClass('in-active-path');
        ...
        _renderMultiple: function () {
            ...
            // 多选框
            let $checked = $('<label class="el-checkbox"><span class="el-checkbox__input"><span class="el-checkbox__inner"></span><input type="checkbox" aria-hidden="false" class="el-checkbox__original" value=""></span></label>');
        ...
        _renderMultipleCheckStrictly: function () {
            ...
            let self = this;
            let cascader = this.cascader;
            let checkedNodeIds = cascader.data.checkedNodeIds;
            let checkedNodes = cascader.data.checkedNodes;
            let nodeId = this.nodeId;
            let parentNode = this.parentNode;
            ...
            // 渲染
            let exist = checkedNodes.some(function (node) {
        ...
        transferParent: function (callback, advance, self) {
            ...
            if (this !== self || advance) {
                let goOn = callback && callback(this);
                if (goOn === false) {
                    return;
                }
            }
        ...
        selectedValue: function () {
            ...
            if (!multiple && (leaf || checkStrictly)) {
                ...
                let paths;
                if (checkStrictly) {
                    let index = checkedNodeIds.indexOf(nodeId);
                    if (index === -1) {
                    ...
                    let allLeafChildren = this.getAllLeafChildren();
                    let checked;
                    ...
                    if (checked === 1) {
                        ...
                        // 未选中、部分选中->选中
                        let add = allLeafChildren.filter(function (node) {
                            return checkedNodeIds.indexOf(node.nodeId) === -1;
                        });
                        paths = checkedNodes.concat(add);
                    }
                }
                let nodeIds = paths.map(function (node) {
                    return node.nodeId;
                });
        ...
        _liLoad: function (event, callback) {
            let leaf = this.leaf;
            let lazy = this.props.lazy;
            let lazyLoad = this.props.lazyLoad;
        ...
        _sync: {
            ...
            syncRadio: function (self) {
                let $li = self.$li;
                let fromIcon = self.icons.from;
                let okIcon = self.icons.ok;
                let multiple = self.props.multiple;
                let checkStrictly = self.props.checkStrictly;
                let nodeId = self.nodeId;
                if (!$li || multiple || checkStrictly) {
                    return;
                }
                let activeNode = self.cascader.data.activeNode;
                if (self.activeNodeId === nodeId) {
                    let ok = $li.find('.' + okIcon);
                    if (ok.length === 0) {
                        $li.prepend('<i class="' + fromIcon + ' ' + okIcon + ' el-cascader-node__prefix"></i>');
                    }
                } else {
            ...
            syncMultiple: function (self) {
                ...
                let checkStrictly = self.props.checkStrictly;
                let multiple = self.props.multiple;
                let disabledFixed = self.config.disabledFixed;
                if (!multiple || checkStrictly) {
                    return;
                }
                let allLeafChildren = self.getAllLeafChildren(disabledFixed);
                // 全部未选中 0
                // 全部选中 1
                // 部分选中 2
                let checked = self._getMultipleChecked(allLeafChildren);
                self._checked = checked;
                if (!$li) {
                    return;
                }
                let $checkbox = self.$checked.find('.el-checkbox__input');
                if (checked === 0) {
            ...
            syncMultipleCheckStrictly: function (self) {
                ...
                let checkedNodes = self.cascader.data.checkedNodes;
                let checkedNodeIds = self.checkedNodeIds;
                let nodeId = self.nodeId;
                let exist = checkedNodes.some(function (node) {
                    return node.path.some(function (node) {
                        return node.nodeId === nodeId;
        ...
        expandPanel: function () {
            ...
            path.forEach(function (node, index, array) {
                if (index !== array.length - 1) {
                    let childrenNode = node.childrenNode;
        ...
        _getMultipleChecked: function (leafNodes) {
            let cascader = this.cascader;
            let checkedNodeIds = cascader.data.checkedNodeIds;
            let allLeafIds = leafNodes.map(function (node) {
                return node.nodeId;
            });
            // 全部未选中 0
            // 全部选中 1
            // 部分选中 2
            let checked = 0;
            for (let i = 0; i < allLeafIds.length; i++) {
                let child = allLeafIds[i];
                if (checked === 2) {
                    break;
                }
    ...
    function Cascader(config) {
        this.config = $.extend(true, {
            ...
            url: '',                    // 动态加载options数据接口URL
            urlMethod: 'GET',           // 动态加载options数据请求方式
            urlLoad: function (ret) {   // 整理动态加载options数据接口URL返回数据回调方法
                if (ret && ret.code == 1) {
                    return ret.data
                }
    ...
    Cascader.prototype = {
         ...
         * URL加载options数据
         */
        _urlLoadData: async function () {
            // 存在数据动态加载URL，动态加载options数据
            if (this.config.url) {
                const ret = await http.ajax({
                    url: this.config.url,
                    method: this.config.urlMethod,
                })

                const options = this.config.urlLoad(ret)
        ...
        _resetXY: function () {
            ...
            if ($panel) {
                ...
                let boundingClientRect = $div[0].getBoundingClientRect();
                let $arrow = $panel.find('.popper__arrow');

                // 距离右边界的偏差值
                let offsetDiff = Math.min(windowWidth - boundingClientRect.x - panelWidth - 5, 0);

                let bottomHeight = windowHeight - (boundingClientRect.top + divHeight);
                if (bottomHeight < panelHeight && boundingClientRect.top > panelHeight + 20) {
                    ...
                    // 距离底部边界的偏差值
                    let yOffset = Math.max(panelHeight - (windowHeight - boundingClientRect.y - divHeight - 15), 0);
                    // 向下
                    $panel.css({
        ...
        _initInput: function () {
            ...
            let fromIcon = this.icons.from;
            let downIcon = this.icons.down;
            let multiple = this.props.multiple;
            let extendClass = this.config.extendClass;
            let extendStyle = this.config.extendStyle;
            ...
            let element = this._initHideElement($e);
            // 在后面插入元素
        ...
        _initFilterableInputEvent: function () {
            let filterable = this.config.filterable;
            if (!filterable) {
                return;
            }
            let timeoutID;
            ...
            let placeholder = this.config.placeholder;
            let beforeFilter = this.config.beforeFilter;
            let filterMethod = this.config.filterMethod;
            let checkStrictly = this.props.checkStrictly;
            let self = this;
            ...
            function filter(event) {
                ...
                timeoutID = setTimeout(function () {
                    ...
                    if (typeof beforeFilter === 'function' && beforeFilter(val)) {
                        ...
                        let filterNodes = nodes.filter(function (node) {
                            let disabled;
                            if (checkStrictly) {
        ...
        _initPanel: function () {
            let $panel = this.$panel;
            let popperClass = this.config.popperClass || '';
            if (!$panel) {
        ...
        _appendMenu: function (nodes, level, parentNode, _menuItem) {
            ...
            let $div = $('<div class="el-scrollbar el-cascader-menu" role="menu" id="cascader-menu"><div class="el-cascader-menu__wrap el-scrollbar__wrap" style="margin-bottom: -17px; margin-right: -17px;"><ul class="el-scrollbar__view el-cascader-menu__list"></ul></div></div>');
            // 重新添加菜单
            this.$panel.find('.el-cascader-panel').append($div);
            // 渲染细项
            this._appendLi($div, nodes);
            let menuItem = { nodes: nodes, level: level, parentNode: parentNode, scrollbar: { top: 0, left: 0 } };
            if (_menuItem) {
        ...
        _appendLi: function ($menu, nodes) {
            let $list = $menu.find('.el-cascader-menu__list');
            if (!nodes || nodes.length === 0) {
                let isEmpty = this.config.empty;
        ...
        refreshMenu: function () {
            ...
            let data = this.data.menuData.concat([]);
            let self = this;
        ...
        _initSuggestionPanel: function () {
            ...
            let $suggestionPanel = this.$suggestionPanel;
            if (!$suggestionPanel) {
        ...
        _setSuggestionMenu: function (nodes) {
            let $suggestionPanel = this.$suggestionPanel;
            let $list = $suggestionPanel.find('.el-cascader__suggestion-list');
            $list.empty();
        ...
        initNodes: function (data, level, parentNode) {
            ...
            for (let key in data) {
                let datum = data[key];
                let node = new Node(datum, this, level, parentNode);
        ...
        _setActiveValue: function (nodeId, node) {
            if (this.data.activeNodeId !== nodeId) {
                let activeNode = this.data.activeNode;
        ...
        _setCheckedValue: function (nodeIds, nodes) {
            let checkedNodes = this.data.checkedNodes;
            let maxSize = this.config.maxSize;
            let maxSizeMode;
        ...
        setValue: function (value) {
            ...
            let strictMode = this.props.strictMode;
            if (strictMode) {
                if (!Array.isArray(value)) {
                    throw new Error("严格模式下,value必须是一个包含父子节点数组结构.");
                }
            }
            let nodes = this.getNodes(this.data.nodes);
            let checkStrictly = this.props.checkStrictly;
            let multiple = this.props.multiple;
            let disabledFixed = this.config.disabledFixed;
            if (multiple) {
                let paths = nodes.filter(function (node) {
        ...
        _initScrollbar: function ($menu, menuItem) {
            let $div = $('<div class="el-scrollbar__bar is-onhoriztal"><div class="el-scrollbar__thumb" style="transform: translateX(0%);"></div></div><div class="el-scrollbar__bar is-vertical"><div class="el-scrollbar__thumb" style="transform: translateY(0%);"></div></div>');
            $menu.append($div);
            let vertical = $($div[1]).find('.el-scrollbar__thumb');
            let onhoriztal = $($div[0]).find('.el-scrollbar__thumb');
            let scrollbar = $menu.find('.el-scrollbar__wrap');
            let $panel = this.$panel;
            let $lis = $menu.find('li');
            let height = Math.max($panel.height(), $menu.height());
            let hScale = (height - 6) / ($lis.height() * $lis.length);
            let wScale = $panel.width() / $lis.width();
            ...
            vertical.mousedown(function (event) {
                ...
                let mousemove = function (event) {
                    event.stopImmediatePropagation();
                    let number = scrollTop + (event.clientY - y) / hScale;
        ...
        _fillingPath: function () {
            ...
            let showAllLevels = this.config.showAllLevels;
            let separator = this.config.separator;
            let collapseTags = this.config.collapseTags;
            let $inputRow = this.$inputRow;
            let placeholder = this.config.placeholder;
            let self = this;
            if (!multiple) {
                let activeNode = this.data.activeNode;
                let path = activeNode && activeNode.path || [];
                if (showAllLevels) {
                ...
                $inputRow.css('height', '');
                let checkedNodes = this.data.checkedNodes;
                let minCollapseTagsNumber = Math.max(this.config.minCollapseTagsNumber, 1);
                if (checkedNodes.length > 0) {
                    let tags = [];
                    let paths = checkedNodes;
                    if (collapseTags) {
                        // 折叠tags
                ...
                let tagHeight = self.$tags.height();
                let inputHeight = $inputRow.height();
                if (tagHeight > inputHeight) {
                    // TODP 修正设置input高度，使得 input 高度过低的问题
        ...
        get$tag: function (label, showCloseIcon) {
            let fromIcon = this.icons.from;
            let closeIcon = this.icons.close;
            let icon = showCloseIcon ? '<i class="el-tag__close el-icon-close ' + fromIcon + ' ' + closeIcon + '"></i>' : '';
        ...
        clearCheckedNodes: function (force) {
            ...
            if (multiple) {
                ...
                if (!force && disabledFixed) {
                    //禁用项被固定，则过滤出禁用项的选值出来
                    let checkedNodes = this.data.checkedNodes;
                    let disNodes = checkedNodes.filter(function (node) {
                        return node.disabled;
                    });
                    let nodeIds = disNodes.map(function (node) {
                        return node.nodeId;
                    });
    ...
    let thisCas = function () {
        ...
        return {
            ...
            expandNode: function (value) {
                let nodes = self.getNodes(self.data.nodes);
                for (let i = 0; i < nodes.length; i++) {
                    let node = nodes[i];
                    if (node.value === value) {
                        node.expandPanel();
    ...
    exports(MODULE_NAME, function (option) {
        let ins = new Cascader(option);
