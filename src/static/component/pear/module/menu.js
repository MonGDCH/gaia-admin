layui.define(['table', 'jquery', 'element', 'http'], function (exports) {
	"use strict";
	...
	layui.link(layui.cache.base + 'pear/css/module/menu.css')
	...
	pearMenu.prototype.render = function (opt) {
		...
		if (option.async) {
			// 异步加载菜单数据
			let symbol = option.url.indexOf('?') !== -1 ? '&' : '?';
			http.ajax({
				url: option.url + symbol + "fresh=" + Math.random(),
		...
		$.each(option.data, function (i, item) {
			...
			if (i === option.defaultMenu) {
				...
				menuItem = '<ul  pear-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree pear-nav-tree" ' + (option.accordion ? "lay-accordion" : "") + ' >';
